#!/usr/bin/env python3
"""
Parallel Evaluation Script for TLDR (Summarization)

This script runs inference-only evaluation for the Parallel (naive concatenation)
configuration where 2 agents generate summaries simultaneously with NO communication.

Agent 1 (Summary): Creates a concise summary
Agent 2 (Elaboration): Creates a detailed summary (2-3x longer)
...
def evaluate_parallel(
    ...
    """
    Run parallel evaluation on TLDR dataset.
    ...
    for prob_idx, item in enumerate(dataset):
        ...
        prompt_agent2 = elaboration_agent_formatter(item)
        ...
        for attempt_idx in range(num_attempts):
            ...
            # Agent 2: Generate detailed summary (in parallel, no communication)
    ...
    # Save CSV
    csv_path = os.path.join(output_dir, f"parallel_results_{timestamp}.csv")
    ...
    json_path = os.path.join(output_dir, f"parallel_summary_{timestamp}.json")
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("PARALLEL EVALUATION SUMMARY (TLDR)")
    print(f"{'='*60}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Parallel evaluation for TLDR (Summarization)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="evals/configs/tldr_parallel_config.yaml",
